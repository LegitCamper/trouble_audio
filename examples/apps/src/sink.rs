...
use trouble_audio::pacs::PacsStorage;
...
/// Backing storage for [`PeripheralConfig`]'s characteristics - the library's own right-sized
/// storage types, kept separate from `PeripheralConfig` itself since it's pure scratch space with
/// no meaningful value of its own. Declare one alongside your config and pass both to
/// [`PeripheralConfig::build_server`].
#[derive(Default)]
pub struct PeripheralConfigStorage<const MAX_ASES: usize> {
    pacs: PacsStorage,
...
impl<'a> PeripheralConfig<'a> {
    ...
    pub fn build_server<'s, const MAX_ASES: usize, const CONNECTIONS_MAX: usize, M: RawMutex>(
        &'s self,
        storage: &'s mut PeripheralConfigStorage<MAX_ASES>,
        ...
        ServerBuilder::<MAX_ASES, CONNECTIONS_MAX, M>::new(self.device_name, &self.appearance)
            .add_pacs(
                self.sink_pac.as_ref(),
                self.sink_audio_locations.as_ref(),
                self.source_pac.as_ref(),
                self.source_audio_locations.as_ref(),
